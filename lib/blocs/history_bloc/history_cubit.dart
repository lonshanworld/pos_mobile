...
class HistoryCubit extends Cubit<HistoryState> {
  ...
  Future<void> _initHistoryList() async {
    final historyList = await PosRepository.instance.readWithMode(
      local: () => LocalPosRepository.getHistoryList(),
      remote: () => PosRepository.instance.fetchHistory(),
    );
  ...
  List<UserModel> getUserModelHistoryList() {
    ...
    for (var element in state.historyList) {
      ...
      if (element.modelType == ModelType.userModel) {
        if (element.oldData != null && element.oldData != "")
          userModelHistoryList.add(
            UserModel.fromJson(jsonDecode(element.oldData!)),
          );
        if (element.newData != null && element.newData != "")
          userModelHistoryList.add(
            UserModel.fromJson(jsonDecode(element.newData!)),
          );
      }
    }
