...
class TransactionStockInDbStorage {
  static Future<void> onCreate(Database db) async {
    ...
    await db.execute(
      "CREATE INDEX IF NOT EXISTS idx_stockIn_createTime ON ${TxtConstants.stockInTableName}(createTime);",
    );
    await db.execute(
      "CREATE INDEX IF NOT EXISTS idx_stockIn_activeStatus ON ${TxtConstants.stockInTableName}(activeStatus);",
    );
  }
  ...
  static Future<List<dynamic>> getAllStockInList(
    Database db, {
    int limit = 100,
    int offset = 0,
  }) async {
  ...
  static Future<int> insertStockIn({
    ...
    return await db.rawInsert(
      ...
      [
        createPersonId,
        dateTime.toString(),
        CodeGenerator.getUniqueCodeForStockIn(),
      ],
    );
  }
}
